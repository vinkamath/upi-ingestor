...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!tx.category) return Response.json({ error: 'Cannot republish without a category' }, { status: 400 })
  ...
  return Response.json({ ok: true, message: 'Transaction republished successfully.' })
}
import { getUser } from '@/lib/db/server'
import { publishers } from '@/lib/publishers'
