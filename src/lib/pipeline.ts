  ...
  for (const tx of txs) {
    ...
    if (!categorization.category) {
      ...
      if (tgLink?.chat_id) {
        ...
        const buttons = (categories ?? []).map((c) => [{ text: c.category, callback_data: `cat:${inserted.id}:${c.category}` }])
        const sent = await sendTelegramMessage(
          tgLink.chat_id,
          `Uncategorized transaction: INR ${tx.amount} at ${tx.merchantRaw}. Pick a category:`,
          { inline_keyboard: [...buttons, [{ text: 'Type new category', callback_data: `cat:${inserted.id}:__manual__` }]] }
        )
