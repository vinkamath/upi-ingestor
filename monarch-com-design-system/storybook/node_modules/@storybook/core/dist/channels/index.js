...
var { CHANNEL_OPTIONS: po, CONFIG_TYPE: uo } = S, Qo = B;
function Zo({ page: r, extraTransports: t = [] }) {
  let e = [new Y({ page: r }), ...t];
  if (uo === "DEVELOPMENT") {
    let o = window.location.protocol === "http:" ? "ws" : "wss", { hostname: s, port: l } = window.location, { wsToken: c } = po || {}, i = `${o}\
://${s}:${l}/storybook-server-channel?token=${c}`;
    e.push(new X({ url: i, onError: /* @__PURE__ */ a(() => {
    }, "onError"), page: r }));
  }
