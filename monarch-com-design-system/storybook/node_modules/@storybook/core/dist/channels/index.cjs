...
tion]";
    ...
    "node_modules/get-intrinsic/index.js"(t, e) {
      "use strict";
      var r, n = SyntaxError, a = Function, s = TypeError, l = /* @__PURE__ */ o(function(O) {
        try {
      ...
      }() : i, u = require_has_symbols()(), p = Object.getPrototypeOf || function(O) {
        return O.__proto__;
      ...
      Re = /\\(\\)?/g, je = /* @__PURE__ */ o(function(g) {
        ...
        var E = [];
        return fe(g, Ne, function(A, M, S, Y) {
          E[E.length] = S ? fe(Y, Re, "$1") : M || A;
        }), E;
      }, "stringToPath3"), De = /* @__PURE__ */ o(function(g, b) {
        var m = g, E;
        if (j(T, m) && (E = T[m], m = "%" + E[0] + "%"), j(v, m)) {
          var A = v[m];
          if (A === y && (A = w(m)), typeof A > "u" && !b)
      ...
      e.exports = /* @__PURE__ */ o(function(g, b) {
        ...
        var m = je(g), E = m.length > 0 ? m[0] : "", A = De("%" + E + "%", b), M = A.name, S = A.value, Y = !1, re = A.alias;
...
ke(ct, {
  ...
  createBrowserChannel: () => lt,
  default: () => st
});
module.exports = Ge(ct);
var Ie = I(te(), 1);
...
var Oe = require("@storybook/core/client-logger");
var Ae = /* @__PURE__ */ o((t) => {
  let e = Array.from(
  ...
  return n.length > 0 && Oe.logger.error("found multiple candidates for event source"), null;
}, "getEventSourceUrl");
...
pan style="color: #FFAE00">${a.type}</span>`;
        if (s && (a.refId = s), a.source = this.config.page === "preview" ? e.origin : Ae(e), !a.source) {
          k.pretty.error(
...
// src/channels/index.ts
var { CHANNEL_OPTIONS: ot, CONFIG_TYPE: it } = Ie.global, st = $;
function lt({ page: t, extraTransports: e = [] }) {
  let r = [new G({ page: t }), ...e];
  if (it === "DEVELOPMENT") {
    let a = window.location.protocol === "http:" ? "ws" : "wss", { hostname: s, port: l } = window.location, { wsToken: c } = ot || {}, i = `${a}\
://${s}:${l}/storybook-server-channel?token=${c}`;
    r.push(new W({ url: i, onError: /* @__PURE__ */ o(() => {
    }, "onError"), page: t }));
  }
